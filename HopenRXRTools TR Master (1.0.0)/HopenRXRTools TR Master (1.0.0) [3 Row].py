...
print("""--HopenRXRTools TR Master (3 Row Edition) """+VERSION+"""--""")
...
new_html = """<td valign="bottom" width="33%"><p align="center"><a href="""+Filenamefix2+Town_Name+Filenamefix2+"""><img src="""+Filenamefix2+Town_Image+Filenamefix2+""" alt="" style="border: 0px solid ;" hspace="0"></a></p>
      <p align="center">&nbsp;</p>
...
Header_Fix = index.find("""<td height="120">""")
if Header_Fix != -1:
    Header_Fix = 0
else:
    Header_Fix = 1
...
for i in range(len(index)):
    j = index.find(sub_string,start_index)
    if(j!=-1):
        if removehalf == 1 and count_er>=2:
...
sub_string3 = """colspan="3" """
Iscolspan=""
start_index3=0
width_html = """width="33%" """

IndexValue = input("Insert Where? ")
...
for i in range(len(index)):
    j = index.find(sub_string3,start_index3)
    if(j!=-1):
        start_index3 = j+1
        Iscolspan = start_index3
        try:
            index = index.replace("""colspan="3" """, """width="33%" """)
...
for i in range(len(new_list)):
  ...
  <tr>"""
    ...
    trrow = trrow+1
if trrow != 1:
    for i in range(4-trrow):
        if trrow != 3 or removehalf == 1:
            final_list = final_list+"""<td valign="bottom"><p align="center">&nbsp;</p></td>"""

if removehalf == 1 and trrow == 4:
    td_end_list.append(td_end_start_index+4)
    td_list.append(td_start_index-1)
    new_list.append(index[td_list[len(td_list)-2]:td_end_list[len(td_list)-2]])
    final_list = final_list+"""</tr>
  <tr>"""
...
greg = index[0:tr_list[0]+(td_list[0]-tr_list[0]-1)]+final_list+index[tr_list[len(tr_list)-1]:len(index)]
